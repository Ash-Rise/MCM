...
针对来袭导弹、无人机与固定目标之间的烟幕干扰弹投放问题，本文建立“确定性三维运动学—完整目标视线几何—时间区间测度”模型。真目标保留为半径 7 m、高 10 m 的有限圆柱；逐时刻计算烟幕云团到导弹—目标表面全部视线线段的最大距离，以不超过 10 m 判定完整遮蔽。多弹效果按有效区间并集计量；问题 5 按每枚烟幕弹的单一指派对象分别计分。
...
问题 3 中三弹共享 FY1 的航向和速度，且相邻投放至少间隔 1 s，故同时优化共享航迹与三组时序，直接最大化区间并集。三段区间首尾交叠，形成 $[5.0593,11.7490]$ s 的连续遮蔽链，并集时长 6.6897 s；多弹作用是延长并集，而非逐弹追求最长区间。

问题 4 的三架无人机没有共享航迹或投放间隔约束，耦合仅来自区间重叠。分别搜索单弹候选后核对并集，所得三个区间两两分离，空档为 8.2721 s 和 15.6691 s，故在该候选邻域内并集等于分项之和，最终为 11.3975 s；若区间重叠，独立组合条件即失效。

问题 5 中同机多弹共享航迹、投放受间隔约束，且边际收益取决于已有覆盖和指派对象，不能拼接单弹最优。由固定航迹配置出发联合调整航迹、时序与指派，正式目标由 30.9212 s 提高到 31.0596 s；最终使用 11 弹，M1、M2、M3 分别获得 20.9604 s、7.6722 s 和 2.4270 s。多分辨率表面采样、端点复算、多起点和局部扰动用于复核；结果仅为记录设置下的较优可行方案，不作全局最优证明。
...
| 烟幕弹 | 共享航迹 $\theta/v$ | 时序 (s) | 投放点 $\mathbf r$/起爆点 $\mathbf e$ (m) | 区间起/止 (s) |
|:---:|:---:|:---:|:---|:---|
| 1 | $179.617493^\circ$<br>131.324813 m/s | $\tau=0.052704$<br>$\delta=3.516856$<br>$b=3.569560$ | $(17793.079,0.046,1800.000)$<br>$(17331.239,3.129,1739.395)$ | 5.059331<br>8.864737 |
| 2 | $179.617493^\circ$<br>131.324813 m/s | $\tau=2.701809$<br>$\delta=4.581549$<br>$b=7.283358$ | $(17445.193,2.369,1800.000)$<br>$(16843.536,6.385,1697.146)$ | 8.253530<br>10.596924 |
| 3 | $179.617493^\circ$<br>131.324813 m/s | $\tau=4.456293$<br>$\delta=5.461029$<br>$b=9.917322$ | $(17214.791,3.907,1800.000)$<br>$(16497.639,8.695,1653.868)$ | 9.917322<br>11.749033 |
...
| 无人机 | 航迹 $\theta/v$ | 时序 (s) | 投放点 $\mathbf r$/起爆点 $\mathbf e$ (m) | 区间起/止/时长 (s) |
|:---:|:---:|:---:|:---|:---|
| FY1 | $5.144284^\circ$<br>139.9999998 m/s | $\tau=0.805429$<br>$\delta=0.122015$<br>$b=0.927444$ | $(17912.306,10.111,1800.000)$<br>$(17929.319,11.642,1799.927)$ | 0.927531<br>5.515586<br>4.588055 |
| FY2 | $306.752521^\circ$<br>135.787385 m/s | $\tau=8.369496$<br>$\delta=4.225056$<br>$b=12.594553$ | $(12680.019,489.427,1400.000)$<br>$(13023.304,29.756,1312.530)$ | 13.787732<br>17.747016<br>3.959284 |
| FY3 | $75.927679^\circ$<br>105.318704 m/s | $\tau=28.734108$<br>$\delta=1.210216$<br>$b=29.944324$ | $(6735.819,-64.580,700.000)$<br>$(6766.810,59.054,692.823)$ | 33.416073<br>36.266194<br>2.850121 |
...
| 烟幕弹/指派 | 时序 (s) | 投放点 $\mathbf r$/起爆点 $\mathbf e$ (m) | 区间起/止 (s) |
|:---:|:---:|:---|:---|
| FY1-1 / M1 | $\tau=0.200185$<br>$\delta=3.395112$<br>$b=3.595297$ | $(17778.925,0.349,1800.000)$<br>$(17421.496,6.260,1743.519)$ | 3.682524<br>7.267552 |
| FY1-2 / M1 | $\tau=1.892864$<br>$\delta=3.772208$<br>$b=5.665072$ | $(17600.724,3.296,1800.000)$<br>$(17203.596,9.863,1730.275)$ | 5.723989<br>9.372007 |
| FY2-1 / M2 | $\tau=6.275219$<br>$\delta=2.085535$<br>$b=8.360754$ | $(12273.070,671.845,1400.000)$<br>$(12363.823,429.847,1378.688)$ | 10.153862<br>14.060737 |
| FY2-2 / M1 | $\tau=7.276147$<br>$\delta=4.715426$<br>$b=11.991573$ | $(12316.626,555.701,1400.000)$<br>$(12521.820,8.539,1291.047)$ | 22.077234<br>25.069020 |
| FY2-3 / M3 | $\tau=12.382770$<br>$\delta=3.259448$<br>$b=15.642218$ | $(12538.843,-36.854,1400.000)$<br>$(12680.680,-415.069,1347.942)$ | 15.642218<br>17.858739 |
| FY3-1 / M3 | $\tau=27.963946$<br>$\delta=0.152409$<br>$b=28.116355$ | $(6684.823,-178.254,700.000)$<br>$(6688.555,-162.875,699.886)$ | 28.116355<br>28.326879 |
| FY3-2 / M1 | $\tau=28.966584$<br>$\delta=1.321390$<br>$b=30.287974$ | $(6709.377,-77.082,700.000)$<br>$(6741.737,56.255,691.444)$ | 34.177564<br>36.988852 |
| FY3-3 / M1 | $\tau=30.108137$<br>$\delta=0.051545$<br>$b=30.159682$ | $(6737.333,38.109,700.000)$<br>$(6738.596,43.310,699.987)$ | 36.947140<br>39.582790 |
| FY4-1 / M2 | $\tau=4.428190$<br>$\delta=9.603351$<br>$b=14.031542$ | $(11350.174,1497.723,1800.000)$<br>$(12109.591,408.443,1348.101)$ | 15.173288<br>18.938588 |
| FY4-2 / M1 | $\tau=6.855530$<br>$\delta=10.601909$<br>$b=17.457440$ | $(11542.124,1222.397,1800.000)$<br>$(12380.505,19.853,1249.238)$ | 19.113722<br>22.875761 |
| FY5-1 / M1 | $\tau=14.160192$<br>$\delta=0.927528$<br>$b=15.087720$ | $(12901.199,-97.596,1300.000)$<br>$(12894.727,27.016,1295.784)$ | 15.087720<br>18.998121 |
...
| 导弹 | 指派区间并集 (s) | $L_j$ (s) |
|:---:|:---|---:|
| M1 | $[3.682524,9.372007]\cup[15.087720,18.998121]$<br>$\cup[19.113722,25.069020]\cup[34.177564,39.582790]$ | 20.960407 |
| M2 | $[10.153862,14.060737]$<br>$\cup[15.173288,18.938588]$ | 7.672175 |
| M3 | $[15.642218,17.858739]$<br>$\cup[28.116355,28.326879]$ | 2.427044 |
...
![图3  问题5按导弹分组的时间配置与资源指派](../figures/fig3_q5_timeline_allocation.png){width=75%}
