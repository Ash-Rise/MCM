...
设站点$j$固定备用车辆数为$r_j$。为保证各站至少保留一辆常规车，备用配置与释放阈值的候选域定义为

$$
\mathcal R=\left\{\boldsymbol r\in\mathbb Z_+^6:\ 0\le r_j\le u_j-1,\ \sum_{j=1}^{6}r_j\ge1\right\},
\qquad
\mathcal T=\{4,5,6,7,8\},
$$

其中$u=(3,2,2,2,1,2)$为任务一确定的站点配车向量，$\tau\in\mathcal T$的单位为min。$\mathcal R$包含47种非零备用向量，因而参数集合$\Theta_C=\mathcal R\times\mathcal T$共有235个候选。

记$\mathcal V^{\mathrm{reg}}(\boldsymbol r)$为常规车辆集合，$W_a(t\mid S(t))$为状态$S(t)$下车辆$a$从时刻$t$起还需等待的时间；该函数同时考虑已有任务与当日12次接单上限。队首呼叫$e$在时刻$t$等待派车时，最快常规车从呼叫到达起的预计响应为

$$
\widehat T_e^{\mathrm{reg}}(t)=t-t_e+
\min_{a\in\mathcal V^{\mathrm{reg}}(\boldsymbol r)}
\left[W_a(t\mid S(t))+3+\frac{60d_{i(e),j(a)}}{45}\right].
$$

设$\mathcal A_e^{R}(t)$和$\mathcal A_e^{\mathrm{reg}}(t)$分别为当前空闲、且接单后不超过日上限的备用车和常规车集合，其车辆数记为$n_e^R(t)$和$n_e^{\mathrm{reg}}(t)$。若$n_e^R(t)>0$，最快备用车及其实际响应时间分别为

$$
a_e^{R}(t)=\arg\min_{a\in\mathcal A_e^{R}(t)}d_{i(e),j(a)},
\qquad
T_e^{R}(t)=t-t_e+3+\frac{60d_{i(e),j(a_e^{R})}}{45}.
$$

策略C仅在常规响应超过阈值且备用车能够严格缩短响应时解除保护，其派车函数为

$$
a_C^{\ast}(e,t)=
\begin{cases}
a_e^{R}(t), & n_e^R(t)>0,\ \widehat T_e^{\mathrm{reg}}(t)>\tau,\ T_e^{R}(t)<\widehat T_e^{\mathrm{reg}}(t),\\
\displaystyle\arg\min_{a\in\mathcal A_e^{\mathrm{reg}}(t)}d_{i(e),j(a)}, & n_e^{\mathrm{reg}}(t)>0,\\
\mathrm{FCFS}, & \text{其他情形}.
\end{cases}
$$

第三种情形表示呼叫继续停留在FCFS队首，等待下一次车辆状态变化。距离相同时，依次按当日接单数、站点编号和车辆编号破同序。备用车执行任务期间仅转为忙碌状态，任务结束回所属站后自动恢复备用身份。

对每个候选$(\boldsymbol r,\tau)$，先在3个共同随机数复制中计算其相对A的平均响应差$\overline{\Delta T}^{\,C-A}$、4分钟内到达率$\overline P_4^{\,C}$、P95响应$\overline Q_{0.95}^{\,C}$与区域平均响应极差$\overline G^{\,C}$，并按下式作词典序筛选：

$$
(\boldsymbol r^{\ast},\tau^{\ast})=
\operatorname*{lex\,min}_{(\boldsymbol r,\tau)\in\Theta_C}
\left(\overline{\Delta T}^{\,C-A},-\overline P_4^{\,C},
\overline Q_{0.95}^{\,C},\overline G^{\,C}\right).
$$

候选筛选结果见图5，其中每一点代表一个备用向量与阈值组合，横轴为相对A的平均响应差，纵轴为4分钟内到达率，零差虚线表示与A平均响应相同。绿色标记为最终进入样本外评价的候选。筛选得到备用向量$\boldsymbol r^{\ast}=(0,0,1,0,0,0)$、释放阈值$\tau^{\ast}=7$ min，即S3的两辆车中固定一辆承担备用角色。该参数的最优性限于所设筛选样本，后续再通过独立样本评价其运行表现。
...
B相对A平均响应减少0.2711 min，成对95%置信区间为$[-0.3116,-0.2305]$ min；C相对A减少0.0271 min，区间为$[-0.0487,-0.0055]$ min。进一步以复制级成对差$D_m=\overline T_{C,m}-\overline T_{A,m}$计算单侧95%上置信限
