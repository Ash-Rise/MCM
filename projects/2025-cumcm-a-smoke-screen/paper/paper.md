...
由于有效区间的端点由视线束与烟幕球的接触关系决定，目标函数在不同遮蔽区间之间可能出现非光滑变化，本文采用无梯度搜索。问题 2 先在目标—导弹几何关系引导的参数空间中用差分进化搜索候选，再用 Nelder–Mead 方法对直接策略变量作局部细化；问题 3 在共享航迹和投放间隔参数化下进行多起点差分进化；问题 4 对每架无人机独立产生单弹候选后按并集评价；问题 5 先建立有限指派的单弹基础，再对各机共享航迹、三弹时序和指派对象进行坐标式改进。
...
在扩大航迹候选范围的复核中，三枚烟幕弹均保留了正的有效区间。该结果说明第三枚烟幕弹的作用来自时间覆盖的延伸，而不能由逐弹时长简单相加得到。
...
先分别寻找各平台的几何可行候选，再按三段有效区间的时间并集评价组合。该做法保留了不同无人机初始位置和高度差异，且不把不同时间段的遮蔽错误地重叠计数。
...
三段区间之间的空档分别为 8.272146 s 和 15.669057 s，因而不存在跨平台的时间重叠。最终并集为
...
对各平台候选进行多起点和可行局部扰动复核，均未发现超过对应基线的有效时长。该结果支持独立组合策略在当前证据下的稳定性，但不构成对全部多机连续可行域的全局搜索证明。
...
问题 5 先用单弹几何候选建立导弹指派的可行基础，再在共享航迹、三弹投放时序和指派对象上进行坐标改进，最后用完整表面模型复核每一枚烟幕弹的区间。每架无人机的航向和速度在其所有烟幕弹之间共享，最终资源配置如表 6。
