...
事故确认、滚动派车、排队处理和模式退出构成现有车队的事故期调度流程。7.7节评价其响应效果，7.8节进一步讨论容量不足时的临时外援配置。
...
| Step 8：筛选外援层级 | 将0～6辆外援配置到事故区最近站点，在共同呼叫流下计算响应改善和边际避免罚金，据90%累计改善阈值、5 min服务目标及盈亏平衡成本形成分层方案。 |
...
$B_N$下的全市平均响应时间由$H=0.5$ h时的6.1706 min总体增至$H=12$ h时的31.6386 min。$H=8$ h节点高于10 h和11 h节点，这一局部波动可能与不同持续时间对应的最不利起点及随机队列状态有关。10个节点上$B_E-B_N$的全市平均响应时间差介于$[-0.0981,0.0264]$ min，且95%置信区间均包含0。现有结果尚不足以表明$B_E$能够显著改善全市平均响应时间。
...
R6、R2和R1分别表示低、中、高压力区域，其中R1的新增呼叫压力最高。在现有资源条件下，可将$B_E$作为事故期的局部保护规则：事故确认后启动事故感知评分，每次派车均滚动更新，事故解除后恢复策略B的日常评分。事故区局部效应支持这一操作规则，全市平均效应仍与0相容。随着$H$由0.5 h增至12 h，$B_N$下的全市平均响应时间由6.1706 min增至31.6386 min，而各节点$B_E-B_N$的全市平均响应时间差绝对值不超过0.0981 min。固定12辆车和5倍事故强度下，响应恶化主要由容量压力形成，因此需要进一步评价临时外援。

### 7.8 临时外援车辆配置与性价比

对每个事故区域和持续时间，本文设置$m=0,1,\ldots,6$辆临时外援。外援驻扎在事故区代表点最近的现有站点；R8到S4和S6距离相同，车辆按S4、S6交替分配。事故开始时外援已经到达驻点，到站延迟记为0，但每次响应仍计入3 min准备时间和驻点至呼叫点的行驶时间。外援与普通车辆采用相同的45 min任务占用、日12次接单上限和任务后回站规则，并与现有车辆共同执行$B_E$评分。临时停车不占六个站点的永久容量。
...
为比较响应改善与资源投入，记$T_m(H)$和$P_m(H)$分别为完整种子区组内10个单区域事故情景的等权平均响应时间和平均延迟罚金。累计响应改善$G_m$表示相对无外援方案的时间下降，边际改善$g_m$表示新增第$m$辆车带来的下降；$S_m$衡量$m$辆方案取得的改善占6辆方案的比例。单车平均避免罚金$A_m$用于判断首批增援的资源效率，边际盈亏平衡成本$B_m$给出第$m$辆车可接受的最高调配成本。其中$G_m,S_m$定义于$m=0,\ldots,6$，$g_m,A_m,B_m$定义于$m=1,\ldots,6$：
...
图11左侧给出代表时长下的外援数量与全市平均响应时间，右侧给出长时事故中各方案相对6辆方案的累计改善比例。3辆处的竖线和90%横线用于识别统一的响应折中阈值。图中$H=0.5$ h的置信区间基于9个完整种子区组，其余时长基于10个种子。

![](../../figures/result_q3_external_support.png){width=88%}

图11 临时外援数量对全市平均响应时间与长时事故累计改善的影响

10个时长节点上，1辆方案的$A_m$均为各非零方案中的最大值，适合作为外援资源有限时的第一步。在$H=6,8,10,11,12$ h五个长时节点，2辆方案仅取得6辆方案84.42%～89.03%的累计改善，3辆方案达到93.23%～96.30%，因此3辆是90%累计改善规则下的最小统一外援数。若管理目标是使10个时长节点的全市等权平均响应时间均低于5 min，则4辆方案的最差节点仍为5.13 min，5辆方案的最差节点降至4.94 min，5辆是满足该目标的最小统一数量。该结论的适用范围是10个区域情景的全市等权平均值，分区与单次随机情景仍按原始结果分别判断。
...
| 3辆 | 五个长时节点达到6辆方案至少90%的累计响应改善 | $S_2=84.42\%\sim89.03\%$，$S_3=93.23\%\sim96.30\%$ | $H=6,8,10,11,12$ h节点的响应折中层级 |
...
事故确认后，调度中心立即启用$B_E$，以现有12辆车执行事故感知派车，并同步估计事故持续时间和外援成本。外援资源有限时先调用1辆；当调度中心将事故归入长时情景时，可参考6、8、10、11和12 h五个节点的共同结果，将外援提升至3辆；若管理目标要求10个时长节点的全市等权平均响应时间低于5 min，则提升至5辆。每次追加车辆前比较实际调配成本$c_m(H)$与$B_m(H)$，事故解除后停止新增外援并恢复策略B的日常评分。该方案将派车顺序修正、容量扩充和成本判断分成三个连续决策环节，可直接用于调度中心的事故期分层响应。
