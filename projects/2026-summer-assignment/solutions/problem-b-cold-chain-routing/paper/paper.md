...
针对一个中央厨房、3 个区域配送站和 6 家门店组成的冷链配送网络，本文建立固定分区、配送站必经的带时间窗车辆路径模型。三辆车均于 4:30 发车并返厨；目标按词典序先最小化迟到门店数，再最小化运输成本与时间窗事件罚。固定分区后完整方案仅有 $2^3=8$ 种，逐一计算即可得到当前模型下的精确最优解。

正常场景的 A、B、C 车路线分别为 $0\to1\to4\to6\to0$、$0\to2\to5\to8\to0$、$0\to3\to9\to7\to0$；总里程为 $103.8713\ \mathrm{km}$，综合成本为 $415.4852$ 元，6 家门店均按时到达。任务三严格禁用有向弧 $2\to8$，但正常最优路线未使用该弧；重优化后路线和成本不变，增量为 0。统一门店服务时长不超过 $21.4205$ 分钟/店时，该零增量结论保持成立。模型使用欧氏距离、恒定车速和发车前静态重优化，实际应用时需以道路行程时间和真实装卸时长重新标定。
...
| $A_i,T_i$ | 到达门店 $i$、开始服务的时刻 | min |
| $E_i,H_i$ | 门店 $i$ 的早到、迟到事件指示变量 | 0/1 |
| $D(P)$ | 完整方案 $P$ 的总里程 | km |
| $L(P)$ | 完整方案 $P$ 的迟到门店数 | 个 |
| $C(P)$ | 运输费与时间窗事件罚之和 | 元 |
...
所有车辆的初始离开时刻为 $t_0=270$ min。对路线中相邻节点 $i,j$，到达时刻递推为

$$
A_j=T_i+s_i+\tau_{ij},
$$

其中门店服务时长基线 $s_i=0$，配送站和中央厨房停留时长也取 0。对有时间窗的门店，开始服务时刻为
...
$$
E_i=\mathbf 1(A_i<a_i),\qquad H_i=\mathbf 1(A_i>b_i).
$$

### 5.2 候选路线与决策变量

车辆 $k$ 对应配送站 $s_k$ 和门店集合 $C_k=\{i_k,j_k\}$。站点必经、固定分区和闭合路线使其候选集恰为

$$
R_k=\{(0,s_k,i_k,j_k,0),(0,s_k,j_k,i_k,0)\}.
$$

令 $z_{kr}=1$ 表示车辆 $k$ 选择候选路线 $r\in R_k$，否则为 0。每辆车必须且只能选择一条路线：

$$
\sum_{r\in R_k}z_{kr}=1,\qquad k\in K.
$$

因为每家门店仅出现在所属车辆的候选集中，该约束同时保证每店恰好服务一次、需求不可拆分、配送站先于门店访问且车辆最终返厨。
...
$$
\sum_{i\in C_k}q_i\le 60,
$$

$$
\sum_{(i,j)\in r}d_{ij}\le180.
$$

车辆 A、B、C 的载货量分别为 27、32、36 箱，容量约束均有余量。冷链条件作为可行性约束处理：运输温度维持在 -18 ℃ 以下，因而不高于门店要求的 -15 ℃。

### 5.4 词典序目标

对完整方案 $P$，定义迟到门店数、总里程和综合成本：
...
$$
D(P)=\sum_{k\in K}\sum_{r\in R_k}z_{kr}
\sum_{(i,j)\in r}d_{ij},
$$

$$
C(P)=4D(P)+30\sum_i E_i+80\sum_i H_i.
$$

模型按词典序最小化

$$
\min_P\bigl(L(P),C(P)\bigr).
$$

第一层保障保鲜时效，第二层才比较经济成本。该写法避免用任意大权重近似优先级。
...
程序对 8 个组合逐一执行距离计算、时间递推、容量与里程检查，并按 $(L,C)$ 排序。由于全部组合均被覆盖，所得解是本文固定分区模型下的精确全局最优解，而非启发式近似。本题规模无需引入遗传算法、蚁群算法或外部 MILP 求解器。
...
求解器生成 `results/candidate_solutions.csv`、`results/route_schedule.csv`、`results/route_summary.csv`、`results/summary.json` 和 `results/result_summary.md`；绘图脚本生成正文所用 3 张 PNG。
