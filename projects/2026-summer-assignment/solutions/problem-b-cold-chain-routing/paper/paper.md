...
针对一个中央厨房、3 个区域配送站和 6 家门店组成的冷链配送网络，任务一建立固定分区、配送站必经的路线型 0-1 车辆路径模型，按词典序先最小化迟到门店数，再最小化运输成本与时间窗事件罚。固定分区后完整方案仅有 2³=8 种，完整枚举可得到当前模型下的精确最优解。

任务二得到 A、B、C 车路线分别为 0→1→4→6→0、0→2→5→8→0、0→3→9→7→0；总里程为 103.8713 km，综合成本为 415.4852 元，6 家门店均按时到达。任务三将“4:30 发车前已知有向弧 2→8 封闭”作为可复现的静态主场景。枚举结果表明，封路删除 4 个本已迟到的组合，而正常最优路线未使用该弧；重优化后路线和成本不变，增量为 0。统一门店服务时长不超过 21.4205 分钟/店时，该结论保持成立。真正的在途动态重路由还需要事件时刻和车辆状态数据。
