...
针对一个含100块组件的分布式光伏电站，本文利用组件15日发电量、题目给定偏差率及同期气象数据，依次完成故障诊断、全站发电量预测和有限预算下的维修排序。任务一直接按照题目给出的偏差率阈值进行三分类，并以辐照度归一化后的同伴相对输出构造不覆盖主分类的遮挡辅助筛查。结果显示，100块组件中正常、隐裂和热斑分别为70、15和15块；辅助筛查得到3块疑似遮挡组件和12块筛查不稳定组件，这些结果仅用于提示现场复核。任务二以“日”为独立样本单位比较辐照度比例模型、含截距模型和受物理符号约束的天气修正模型，采用逐日留一验证和一标准误差简约规则选择M0辐照度比例模型。该模型的留一平均绝对误差为0.180787 kWh，全15日原尺度拟合系数为51.249391 kWh/(kWh/m²)，据第16日辐照度5.0 kWh/m²得到全站当前运行状态点预测256.246955 kWh；条件95%期望值置信区间为[256.057714, 256.436195] kWh，单日预测区间为[255.654628, 256.839281] kWh。任务三将故障组件按15日历史平均每日绝对可恢复损失排序，在等预算、独立且可加的维修假设下选出PV084、PV044、PV071、PV060、PV076、PV010、PV008、PV051、PV029和PV009，历史平均每日增发量为5.781380 kWh，按加性反事实计算的全站平均发电量提升率为2.968834%。进一步按固定比例将任务二的第16日站级预测传递到该维修集合，得到7.607546 kWh的条件性补充增发量。本文结论适用于题目给定的偏差率定义、15日观测范围和上述维修可加假设，不将筛查提示解释为物理确诊，也不将条件区间解释为精确覆盖率或实际维修效果保证。
...
使用$1.4826\times\operatorname{MAD}$度量时间离散程度。组件的离散度高于正常同伴离散度中位数加3倍稳健尺度，且至少有两天出现同向或相邻的异常，才满足一种参照体系下的疑似遮挡条件。为避免筛查结果过度依赖同伴选择，同时用同配置正常同伴和全部正常组件两种参照分别计算：两种参照均标记时记为“疑似遮挡”，结果不一致时记为“筛查不稳定”，两者均未标记时记为“未发现疑似遮挡”。该筛查规则只提供现场复核优先级，不改变式（1）的类别。
...
|:---:|:---|---:|---:|---:|---:|---:|:---:|:---:|
| M0 | 是 | 0.180787 | 0.237723 | 0.571382 | 0.99999055 | 0.99999221 | 是 | 选中 |
| M1 | 是 | 0.188131 | 0.240464 | 0.563589 | 0.99999033 | 0.99999238 | 是 | 否 |
| M2 | 否 | 0.186859 | 0.244338 | 0.593485 | 0.99999002 | 0.99999221 | 否 | 否 |

M0的留一MAE最小，为0.180787 kWh；以最小MAE加一个标准误得到的阈值为0.222042 kWh，M1的误差略大但仍落入一标准误集合，因此不需要用截距换取难以区分的微小改善。M2虽然在数值上比M1的MAE略低，但温度项全样本估计接近零、在留一拟合中多次落到零边界，风速项在删去单日后变化超过100%，故不具备稳定的参数解释和预测资格。由此选择M0。这个选择来自误差、资格和简约性共同作用，而不是预先指定M0获胜。

图2左图展示M0在15个历史日上的逐日留一预测与实际总量；低辐照日和高辐照日均沿同一比例关系变化，最大留一绝对误差为0.571382 kWh。图2右图单独放大第16日区间，以避免窄区间在全量程图中难以辨认。
...
=51.249390954\times5.0
=256.246954770\ \text{kWh}.
\tag{9}
$$

点预测保持在$Y$尺度上。考虑参数估计不确定性时，采用HC3稳健参数协方差；考虑单日实现波动时，再按辐照度缩放条件残差方差。该异方差处理只改变区间计算，不重新定义或替换式（5）的点预测模型。结果见表4。

**表4 第16日全站发电量预测**

| 输出 | 点预测/kWh | 下界/kWh | 上界/kWh | 解释范围 |
|:---|---:|---:|---:|:---|
| 点预测 | 256.246955 | — | — | M0全15日原尺度最终拟合 |
| 95%期望值置信区间 | — | 256.057714 | 256.436195 | 条件性参数不确定性 |
| 95%单日预测区间 | — | 255.654628 | 256.839281 | 条件性参数与日级残差不确定性 |
| 同一模型自助法敏感性：期望值区间 | — | 256.086728 | 256.405148 | 同一M0定义的原尺度重拟合 |
| 同一模型自助法敏感性：预测区间 | — | 255.801192 | 256.782074 | 同一M0定义的原尺度重拟合 |

预测区间包含单日残差波动，因此宽于期望值置信区间。以同一M0定义在原尺度上进行500次日级残差自助法重拟合，得到的敏感性区间与主区间方向一致，未显示出会改变第16日结论的差异。由于只有15个历史日，上述“95%”是模型条件下的区间标记，不能被解释为精确覆盖率的证明；第16日气象预报本身的误差、候选模型选择不确定性、未来故障状态变化、限发和逆变器事件也未纳入区间。
...
|---:|:---:|:---:|---:|---:|---:|---:|
| 1 | PV084 | 热斑 | -33.1 | 2.088690 | 0.691356 | 0.691356 |
| 2 | PV044 | 热斑 | -29.3 | 2.098727 | 0.614927 | 1.306283 |
| 3 | PV071 | 热斑 | -30.1 | 2.038340 | 0.613540 | 1.919824 |
| 4 | PV060 | 热斑 | -28.8 | 2.088858 | 0.601591 | 2.521415 |
| 5 | PV076 | 热斑 | -28.1 | 2.077979 | 0.583912 | 3.105327 |
| 6 | PV010 | 热斑 | -29.5 | 1.970875 | 0.581408 | 3.686735 |
| 7 | PV008 | 热斑 | -28.7 | 1.976531 | 0.567264 | 4.253999 |
| 8 | PV051 | 热斑 | -26.0 | 2.100901 | 0.546234 | 4.800233 |
| 9 | PV029 | 热斑 | -24.7 | 2.067729 | 0.510729 | 5.310963 |
| 10 | PV009 | 热斑 | -23.7 | 1.984884 | 0.470418 | 5.781380 |

图3显示全部30块故障组件的损失排序、前10名和第10/11名截止点。第10名PV009的中心损失为0.470418 kWh/day，第11名PV070为0.450322 kWh/day。考虑给定偏差率的显示精度，在偏差率上下各扰动0.05个百分点后，二者的损失区间分别为[0.469118, 0.471719]和[0.449015, 0.451631] kWh/day，区间不重叠，因此主名单不会因该显示精度引起的截止点交换而改变。
...
$$
G_{\mathrm{hist}}=\sum_{i\in S_{10}}L_i=5.781380\ \text{kWh/day}.
\tag{14}
$$

当前全站15日平均发电量为194.735733 kWh/day，在完全恢复且收益可加的反事实下，修复后全站平均发电量为

$$
\bar Y_{\mathrm{post}}=194.735733+5.781380
=200.517113\ \text{kWh/day}.
\tag{15}
$$
...
$$
\frac{5.781380}{194.735733}\times100\%=2.968834\%.
\tag{16}
$$
...
=5.781380\times\frac{256.246955}{194.735733}
=7.607546\ \text{kWh}.
\tag{17}
$$

区间也使用同一个固定比例缩放：将任务二期望值置信区间的两个端点分别乘以$G_{\mathrm{hist}}/\bar Y$，得到补充增发量的条件95%区间[7.601928, 7.613164] kWh；将任务二单日预测区间以同样比例缩放，得到[7.589961, 7.625131] kWh。这样传播的只是任务二的站级参数不确定性和条件日级残差波动；维修名单、历史损失和比例关系均保持不变，偏差率舍入、正常反事实误差、维修有效率、组串耦合、加性假设、模型选择和第16日气象预报输入误差均未传播到式（17）。
...
1. 按题目偏差率阈值，100块组件中正常70块、隐裂15块、热斑15块，没有超出三分类定义域的记录。辐照度归一化同伴筛查给出3块疑似遮挡和12块筛查不稳定组件，应作为现场复核提示，不覆盖主诊断。

2. 以站级日总发电量为目标并进行逐日留一比较后，M0辐照度比例模型在合格模型中具有最小留一MAE，且M1的微小误差差异不足以抵消其额外截距，M2的天气项不稳定。M0全15日原尺度拟合给出的第16日当前运行状态点预测为256.246955 kWh，条件95%期望值置信区间为[256.057714, 256.436195] kWh，条件95%单日预测区间为[255.654628, 256.839281] kWh。

3. 在等预算、独立可加和完全恢复的反事实下，优先维修名单为PV084、PV044、PV071、PV060、PV076、PV010、PV008、PV051、PV029和PV009。该集合的历史平均每日可恢复损失为5.781380 kWh，对应全站平均发电量由194.735733 kWh/day提高到200.517113 kWh/day，反事实相对提升率为2.968834%。将该历史增益按同一固定比例映射到第16日，得到7.607546 kWh的条件性补充增发量；这一补充量不改变历史主排序，也不代表实际维修后的保证收益。
