...
**针对任务三**，事故确认后以日常策略B为基础启动事故感知派车。事故区未来45 min预测强度调整为日常值的5倍，并在当前额外行驶时间不超过2 min的候选车辆中滚动选取评分最小者；事故解除后恢复日常模式。事故持续时间设为0.5～12 h的连续变量，由10个自适应节点构造PCHIP响应面及95%置信带。随持续时间由0.5 h增至12 h，全市平均响应时间由6.1706 min增至31.6386 min；事故感知预测的全市平均响应时间差均不显著，仅在持续8、10和11 h时使事故区平均响应时间分别减少0.1519、0.0941和0.1965 min。固定12辆车时，事故时长增长带来的响应恶化远大于预测修正形成的成对差，单独调整预测难以抵消事故期容量压力。
...
任务一要求确定站点启用、车辆配置和区域服务分配，使日均140次呼叫全部得到覆盖，并兼顾出车距离和规划服务覆盖。任务二要求利用历史需求信息构造24 h呼叫过程，设计就近派车和备用车辆配置，计算优化后的平均响应时间。任务三研究某一区域呼叫强度临时升至正常5倍时的系统响应，并在现有12辆车配置下给出动态应急派车方案。
...
平均出车距离与时限覆盖率分别衡量运行效率和空间可达性，二者的量纲及优化方向不同，以覆盖率或响应时间为目标时可能得到不同的站点配置。本文采用词典序目标：加权出车距离居首，在距离最优解集合内比较3 km规划服务覆盖率。
...
任务三以策略B和任务一确定的12辆车配置为常态基线。事故确认后，调度中心将事故区未来45 min预测强度调整为日常值的5倍，在额外行驶时间不超过2 min的候选车辆中滚动计算事故感知评分。评分最低的车辆执行任务；暂无合法空闲车辆时，呼叫按FCFS留队等待。事故解除后恢复日常预测强度和派车评分，站点与车辆配置保持不变。

事故持续时间$H$取连续区间$[0.5,12]$ h，有限仿真节点用于构造数值响应面。每个持续时间对应一个日内强度积分最大的事故窗口。常态预测模式$B_N$与事故感知模式$B_E$共享随机呼叫流，并在同一种子内计算成对差。事故区与非事故区指标按实际呼叫数汇总，零呼叫情景的权重记为0。
...
R6、R2和R1分别表示低、中、高压力区域，其中R1的新增呼叫压力最高。在现有资源条件下，可将$B_E$作为事故期的操作性局部保护规则：事故确认后启动事故感知评分，每次派车均滚动更新，事故解除后恢复策略B的日常评分。事故区局部效应支持这一操作规则，全市平均效应仍与0相容。随着$H$由0.5 h增至12 h，$B_N$下的全市平均响应时间由6.1706 min增至31.6386 min，而各节点$B_E-B_N$的全市平均响应时间差绝对值不超过0.0981 min。固定12辆车和5倍事故强度下，事故时长增长带来的响应恶化远大于预测修正形成的成对差，预测修正的作用主要体现在局部车辆保护。

长时事故下可优先评估增车、临时站或外援等扩容方案。表11和表12保留各$H$下的条件结果；若获得事故持续时间分布，可据此计算事故期总体风险指标。
...
任务三以有限节点近似连续事故时长，并在复制层级构造PCHIP曲线，随机复制之间的差异得以保留并形成置信带。分域效应按实际呼叫数加权，无呼叫情景以零权重保留，各事故区域采用一致的统计口径。全市与事故区效应分别报告，结论强度由相应置信区间限定。
