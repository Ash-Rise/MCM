...
为便于比较各指标的变化幅度，从效率、尾部风险、排队、时限保障、区域公平和经济成本六个方面，选取平均响应、P95响应、平均等待、4分钟内到达率、区域平均响应极差和单次延迟成本作为核心指标。每个指标先对30个复制取均值，再计算相对策略A的有符号优化幅度。平均响应、P95响应、平均等待、区域平均响应极差和单次延迟成本按A的均值减去待评策略均值后除以A的均值；4分钟内到达率按待评策略均值减去A的均值后除以A的均值。由此，正值统一表示优化，负值表示该指标相对A有所下降。各维度只保留一项代表性指标，保持图中各条形的现实含义清晰。
...
| Step 6：开展样本外评价 | 在30个新复制中令A、B、C共享呼叫流，统计响应、等待、队列、公平性和延迟成本，并计算相对A的优化幅度及成对差的95%置信区间。 |
...
其中$`\overline D`$和$`s_D`$分别为30个成对差的样本均值和样本标准差。所得$`U_{0.95}=-0.0091`$ min，小于0，说明在本次30个样本外复制中，所选C方案的平均响应未出现相对A的退化。

图6比较B、C相对A的六项核心指标平均优化幅度。B在六项指标上均为正向变化，其中平均等待降幅最大，为45.11%；单次延迟成本、P95响应、平均响应、4分钟内到达率和区域平均响应极差分别优化12.38%、9.95%、4.83%、4.14%和3.81%。C在平均等待上的优化幅度为9.77%，其余指标变化较小，4分钟内到达率相对A下降0.45%。图7进一步给出各指标成对差的均值和95%置信区间。图6用于比较相对变化幅度，图7用于判断差异的统计不确定性。

<p align="center">
  <img src="../../figures/result_q2_multi_metric.png" width="82%" alt="">
</p>

图6 策略B、C相对A的多指标平均优化幅度
