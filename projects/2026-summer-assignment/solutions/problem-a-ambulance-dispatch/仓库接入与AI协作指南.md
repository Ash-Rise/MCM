...
协作者准备自己编辑时，只需说“我要修改论文”。AI必须读取规则、检查工作区、获取最新 `origin/main`，并在安全时创建 `review/<协作者代号>-edit` 分支，然后打开唯一正文 `.md` 并停止，不得代写、暂存或提交。

协作者编辑结束后说“修改完成”。AI必须先检查差异、显示效果和修改范围，输出增删报告并等待确认。只有协作者再次明确确认提交后，AI才可以 Commit、Push 和创建 PR；AI不得自动 Merge。
