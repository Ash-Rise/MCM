...
| $G_m(H),g_m(H),S_m(H)$ | 累计响应改善、边际响应改善、相对6辆方案的累计改善比例 | min，min，- |
...
设置$B_N$的目的，是为$B_E$构造同一事故情景下的反事实基线。两种模式共享事故区域、起点、持续时间、呼叫数量、到达时刻、需求区域、车辆初始状态和随机种子，差别仅在于派车评分是否采用已识别的5倍事故强度。因此，二者的响应时间差主要来自事故信息对车辆选择顺序的影响，事故规模、呼叫时刻和车辆初始状态等随机因素均得到控制。

直接将$B_E$的事故期响应时间与任务二的常态结果比较时，所得差异同时包含事故新增需求造成的负荷冲击和事故感知派车带来的调整作用，难以判断现有方案效果较差的具体原因。引入$B_N$后，评价链条可以分为三个层次：$B_N$描述事故已经发生而前瞻预测尚未修正时的系统表现；$B_E-B_N$识别事故信息和派车顺序修正的边际作用；增加外援后的方案与$B_E$比较，则识别运力扩充的边际作用。当$B_E-B_N$接近0而外援带来的改善较大时，可以判定容量约束主导了响应恶化，事故信息修正的作用相对有限。
...
其中$(\beta,\delta)=(4,2)$，$C_a^{E}(t)$表示按事故强度重新计算的未来45 min覆盖损失；前瞻窗口与事故时段重叠的部分采用5倍强度。若某辆车离站会显著削弱事故区的后续响应，其评分将上升，从而更可能被保留下来。两种模式采用相同的车辆配置、FCFS规则、45 min占用、日12次上限和任务后回站规则。成对差为负表示事故感知派车改善响应，95%置信区间包含0则表示现有复制尚未提供稳定的全市改善证据。
...
图11左侧给出代表时长下的外援数量与全市平均响应时间，右侧给出长时事故的响应改善达成率，其中6辆外援固定为100%。3辆处的竖线和90%横线用于识别统一的响应折中阈值。图中$H=0.5$ h的置信区间基于9个完整种子区组，其余时长基于10个种子。

![](../../figures/result_q3_external_support.png){width=88%}

图11 临时外援数量对全市平均响应时间与响应改善达成率的影响
...
表13将上述结果压缩为三个可执行层级。实际调度成本尚未给出，表中层级表示响应阈值与资源选择规则；经济最优数量仍由具体调配成本决定。

表13 临时外援的分层决策边界

| 外援层级 | 判定规则 | 结果证据 | 适用说明 |
|:---:|---|---|---|
| 1辆 | 10个时长节点上单车平均避免罚金$A_m$最大 | 10个节点均由$m=1$取得最大$A_m$ | 资源有限时优先启动，代表最高平均边际价值的第一步 |
| 3辆 | 五个长时节点的响应改善达成率均不低于90% | $S_2=84.42\%\sim89.03\%$，$S_3=93.23\%\sim96.30\%$ | $H=6,8,10,11,12$ h节点的响应折中层级 |
| 5辆 | 10个时长节点的全市等权平均响应时间均低于5 min | 4辆方案最差为5.13 min，5辆方案最差为4.94 min | 面向较严格的全市平均服务目标 |

以12 h事故为例，第1～6辆外援的边际盈亏平衡成本$B_m$依次为39.01、19.69、8.24、2.48、0.76和0.19万元/辆·事故情景。若第$m$辆车的实际调配成本$c_m(H)$不超过$B_m(H)$，则该次增援的延迟罚金收益能够覆盖资源成本；超出该阈值时停止追加。
