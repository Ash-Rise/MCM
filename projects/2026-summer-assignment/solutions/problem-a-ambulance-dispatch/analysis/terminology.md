...
第一问的配置、分配矩阵和静态指标已经独立复算。第二问最终取B的 $(\beta,\delta)=(4,2)$ 作为日常主方案；推荐固定备用方案C取 $r=(0,0,1,0,0,0)$、$\tau=7$ min。任务三的连续持续时间、5倍强度、最不利窗口、$B_N/B_E$ 对照、事故期评价边界、复制级响应面和分域成对效应均已冻结；应急模式不与C机械叠加，PCHIP只作为有限节点间的数值近似。
