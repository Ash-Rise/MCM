...
相较于策略A，策略B使平均响应时间降低4.83%、P95响应时间降低9.95%、平均等待时间降低45.11%，平均单次延迟成本和日均延迟总成本分别减少50.64元/次和7089.22元/日，同时减小区域平均响应时间极差。因此，策略B被选为日常主方案。时空随机需求下的车辆配置与调度适合采用仿真优化框架进行比较^\[4\]^，本文的输入和约束均由题目条件确定。
