...
**针对任务三**，分别设置10个区域单独发生事故，令持续时间在0.5～12 h内连续变化，并由10个自适应节点构造PCHIP响应面。固定12辆车时，事故感知预测的全市平均响应时间差在各节点均不显著，仅在持续8、10和11 h时使事故区平均响应时间分别减少0.1519、0.0941和0.1965 min。进一步将0～6辆临时外援部署至事故区最近站点，并在共同呼叫流下完成7000行情景实验。10个时长节点上，1辆外援均具有最高的单车平均避免罚金；在6、8、10、11和12 h五个长时节点，3辆可实现6辆方案93.23%～96.30%的累计响应改善；5辆使10个时长节点的全市等权平均响应时间均低于5 min。由此形成1、3、5辆的分层增援方案，并以新增车辆的边际盈亏平衡成本确定是否继续增援。
...
| $G_m(H),g_m(H),S_m(H)$ | 累计响应改善、边际响应改善、响应改善达成率 | min，min，- |
| $A_m(H),B_m(H)$ | 单车平均避免罚金、第$m$辆外援的边际盈亏平衡成本 | 元/辆·事故情景 |
...
任务三中的响应变化同时受到事故新增需求和调度规则调整的影响。为了判断事故信息进入调度系统后能否改善车辆选择，本文先固定车辆数量和实际事故呼叫，只考察45 min前瞻需求预测是否更新。该比较属于应急方案的消融分析，用于评价事故感知派车本身的作用，完整方案的效果还需结合7.8节的外援实验。本文设置以下两个模式：

- $B_N$：实际呼叫流包含事故增量，但45 min前瞻损失仍按日常强度计算；
- $B_E$：只在事故已经发生的时段内，把已识别事故区的前瞻强度乘子改为5，其余评分、约束和参数保持不变。

这里的$B_N$仍包含事故呼叫，表示事故已经发生、调度预测仍停留在常态的情形。它可以对应事故尚未被识别，或事故信息尚未进入派车评分的运行状态。$B_E$则表示调度中心已经识别事故区域，并把该信息用于评估未来45 min的车辆覆盖损失。因此，$B_N$构成$B_E$在同一事故情景下的反事实基线。

选择$B_N$作为对照的关键在于只改变一个因素。若直接将$B_E$与任务二的常态结果比较，响应差异同时包含事故新增需求造成的负荷冲击和事故感知派车带来的调整作用；若与策略A比较，派车目标、覆盖保护和负荷惩罚也会同时变化；若直接与增加外援后的方案比较，车辆容量又成为新的影响因素。上述比较均无法单独识别事故信息的作用。$B_N$与$B_E$保持车辆配置、实际呼叫、排队规则、任务占用时间和派车参数一致，仅改变前瞻预测强度，因而形成最小干预对照。

仿真中，两种模式在相同事故区域$k$、持续时间$H$和随机种子$s$下共享事故起点、呼叫数量、到达时刻、需求区域及车辆初始状态，并计算成对差$D_{k,s}(H)=T_{k,s}^{B_E}(H)-T_{k,s}^{B_N}(H)$。这种共同随机数设计控制了事故规模、呼叫时刻和初始队列状态造成的波动，使$D_{k,s}(H)$主要反映预测信息改变车辆派出顺序后的响应变化。$D_{k,s}(H)<0$表示事故感知派车改善响应；跨随机种子的95%置信区间包含0时，现有复制尚未提供稳定的改善证据。

由此形成三层评价关系：$B_N$描述事故负荷下、预测尚未修正时的系统表现；$B_E-B_N$识别事故信息与派车顺序修正的边际作用；增加外援后的方案与$B_E$比较，识别运力扩充的边际作用。前一项改善很小而后一项改善明显时，说明现有方案的主要瓶颈来自车辆容量，事故信息修正只能改变有限车辆的使用顺序。这一分解既解释现有12辆车方案为何改善有限，也为后续引入临时外援提供依据。

事故感知模式在计算未来覆盖损失时采用已识别事故区的5倍强度，评分结构为
...
其中$(\beta,\delta)=(4,2)$，$C_a^{E}(t)$表示按事故强度重新计算的未来45 min覆盖损失；前瞻窗口与事故时段重叠的部分采用5倍强度。若某辆车离站会显著削弱事故区的后续响应，其评分将上升，从而更可能被保留下来。两种模式采用相同的车辆配置、FCFS规则、45 min占用、日12次上限和任务后回站规则。

### 7.3 应急响应方案

任务三的应急响应方案由事故感知派车和分层外援组成。事故发生后，调度中心先以现有12辆车启用$B_E$，立即调整派车顺序，同时滚动估计事故持续时间和外援成本，再按7.8节的仿真结果逐级调用外援。具体执行过程如下。
...
3. **启动分层外援。** 事故确认且外援可用时先调用1辆；当持续时间估计对应$H=6,8,10,11,12$ h五个已验证节点时，将外援增至3辆，作为响应改善与投入之间的折中档位；若管理目标要求10个时长节点的全市等权平均响应时间均低于5 min，则增至5辆。估计时长位于相邻节点之间时，按较长一侧的已验证节点查表，作为调度中的保守执行规则，不将其解释为新的仿真结论。增加第$m$辆外援前，将实际调配成本$c_m(H)$与边际盈亏平衡成本$B_m(H)$比较，前者不超过后者时继续增援。
4. **处理车辆不足。** 当前无合法空闲车辆时，呼叫继续留在全市FCFS队列，待车辆释放、外援投入或跨日接单计数重置后再按上述规则派出。事故期到达且结束时仍在队列中的呼叫继续处理，派车次序保持不变。
5. **退出事故模式。** 调度中心收到事故解除信息后，停止新增外援，将事故区预测倍率恢复为1，后续派车回到策略B的日常评分。已投入外援继续处理事故期积压，待事故期到达的排队呼叫清空且已接任务完成后撤回；现有车辆仍返回原所属站点。

上述流程先利用事故信息调整现有车辆，再根据容量缺口追加外援，最后按成本阈值控制增援规模。7.7节评价12辆车条件下的实际效果和容量瓶颈，7.8节给出1、3、5辆外援层级的数值依据。
...
平均响应时间、P95响应时间、4分钟内到达率和最大事故排队呼叫数的数量级不同，选点前按“事故区×指标”分别统一尺度。到达率保持0至1的原尺度，其余指标用当前样本绝对值的90%分位数无量纲化。选点同时考察$B_N$、$B_E$及二者成对差曲线。相邻区间的斜率变化刻画曲线弯折程度，区间端点95%置信半宽的最大值刻画统计不确定性。
...
插值在复制层级完成，以保留10个随机种子之间的波动。对每个事故区、模式、指标和种子分别构造保形分段三次Hermite插值$\widehat M_{k,s}(H)$，再在任意评价位置跨复制求均值。均值曲线为
...
| Step 8：筛选外援层级 | 将0～6辆外援配置到事故区最近站点，在共同呼叫流下计算响应改善和边际避免罚金，以6辆方案的改善为100%，据90%响应改善达成率、5 min服务目标及盈亏平衡成本形成分层方案。 |
...
7.7节表明，现有12辆车下的事故感知派车只能形成局部改善，容量压力仍占主导。为补充事故期运力，本文进一步考察临时外援车辆的数量、响应收益与成本边界。对每个事故区域和持续时间，设置$m=0,1,\ldots,6$辆临时外援。外援驻扎在事故区代表点最近的现有站点；R8到S4和S6距离相同，车辆按S4、S6交替分配。事故开始时外援已经到达驻点，到站延迟记为0，但每次响应仍计入3 min准备时间和驻点至呼叫点的行驶时间。外援与普通车辆采用相同的45 min任务占用、日12次接单上限和任务后回站规则，并与现有车辆共同执行$B_E$评分。临时停车不占六个站点的永久容量。

正式实验覆盖10个事故区域、10个时长节点、10个随机种子和7种外援数量，共得到7000行复制结果。同一区域、时长和种子下的7种外援方案共享呼叫流。$H=0.5$ h时，种子600002在8个单区域事故情景中没有事故窗口呼叫，响应时间因而未定义。分域汇总记录各指标的实际有效样本数；全市结果采用完整种子区组，排除该种子后由9个种子计算，其余时长均为10个种子。零呼叫情景的响应时间保留为未定义值。

为比较响应改善与资源投入，记$T_m(H)$和$P_m(H)$分别为完整种子区组内10个单区域事故情景的等权平均响应时间和平均延迟罚金。累计响应改善$G_m$表示相对无外援方案的时间下降，边际改善$g_m$表示新增第$m$辆车带来的下降。响应改善达成率$S_m$以6辆外援相对无外援取得的时间下降为100%，表示$m$辆外援已经实现其中的比例。单车平均避免罚金$A_m$用于判断首批增援的资源效率，边际盈亏平衡成本$B_m$给出第$m$辆车可接受的最高调配成本。其中$G_m,S_m$定义于$m=0,\ldots,6$，$g_m,A_m,B_m$定义于$m=1,\ldots,6$：

$$
\begin{aligned}
G_m(H)&=T_0(H)-T_m(H), & g_m(H)&=T_{m-1}(H)-T_m(H),\\
S_m(H)&=\frac{T_0(H)-T_m(H)}{T_0(H)-T_6(H)}, &
A_m(H)&=\frac{P_0(H)-P_m(H)}{m},\\
B_m(H)&=P_{m-1}(H)-P_m(H).&&
\end{aligned}
$$

图11(a)给出代表时长下的外援数量与全市平均响应时间，图11(b)给出长时事故的响应改善达成率，其中6辆外援固定为100%；3辆处的竖线和90%横线用于识别统一的响应折中阈值。图11(c)进一步给出新增第$m$辆外援相对$m-1$辆方案所避免的延迟罚金，即$B_m(H)$。由于该指标跨越多个数量级，热力图采用对数色阶，并在单元格内标出以万元/辆·事故情景计的均值。图中$H=0.5$ h的结果基于9个完整种子区组，其余时长基于10个种子。

![](../../figures/result_q3_external_support.png){width=88%}

图11 临时外援数量的响应收益与边际避免罚金

10个时长节点上，1辆方案的$A_m$均为各非零方案中的最大值，适合作为外援资源有限时的第一步。在$H=6,8,10,11,12$ h五个长时节点，2辆方案的响应改善达成率为84.42%～89.03%，3辆方案达到93.23%～96.30%，因此3辆是90%响应改善达成率规则下的最小统一外援数。若管理目标是使10个时长节点的全市等权平均响应时间均低于5 min，则4辆方案的最差节点仍为5.13 min，5辆方案的最差节点降至4.94 min，5辆是满足该目标的最小统一数量。该结论的适用范围是10个区域情景的全市等权平均值，分区与单次随机情景仍按原始结果分别判断。

图11(c)显示，各时长下新增车辆的边际避免罚金均随外援序号增加而下降，反映出明显的边际收益递减。以12 h事故为例，第1～6辆外援的$B_m$依次为39.01、19.69、8.24、2.48、0.76和0.19万元/辆·事故情景。若第$m$辆车的实际调配成本$c_m(H)$不超过$B_m(H)$，该次增援所减少的延迟罚金可以覆盖资源成本；否则停止追加。题目未给出实际调配成本，经济最优数量仍由具体调配成本决定。
...
任务三以有限节点近似连续事故时长，并在复制层级构造PCHIP曲线，随机复制之间的差异得以保留并形成置信带。现有车队的事故感知效应与临时外援的容量收益分别评价，前者说明派车顺序修正的局部作用，后者给出1、3、5辆的分层资源阈值。外援汇总按指标记录实际有效样本数，并以完整10区域种子区组计算全市结果；继续增援的条件由边际盈亏平衡成本表示。
...
进一步获得事故识别时间、强度等级和持续时间分布后，可在仿真中显式加入识别滞后，并按事故发生概率对条件响应面进行滚动风险评价。若补充外援到达时间、调配价格和临时驻点条件，可将最近站点规则扩展为“外援数量—驻点—到达时刻”的联合优化，并按实际成本重新计算边际盈亏平衡边界。
